...
只要 TCP 连接一方收到对方的零窗口通知，就启动持续计时器；一段时间后发送窗口探测报文  
...
发送窗口`swnd`是拥塞窗口`cwnd`和接收窗口`rwnd`中的最小值  
...
IPv6 地址长度是 128 位，是以每 16 位作为一组，每组用冒号 「:」 隔开（`::`表示连续的 0）   
...
客户端只要能够找到任意一台 DNS 服务器，就可以通过它找到根域 DNS 服务器，然后再一路找到位于下层的某台目标 DNS 服务器  
