...
（对于多线程共享数据，只是简单粗暴地关闭对该变量的优化，等于放弃了高速的寄存器和缓存，且没法解决指令重排可能带来的问题）   
...
### 多线程带来的问题
- 非原子操作（一条语句（如`a = b`）可能对应多条汇编语句，甚至尽管只是简单的赋值）
- 编译器优化时的指令重排 与 现代CPU乱序执行机制（不同的`std::thread`分线程执行时，编译器并不能分析出指令重排的后果）
- 多核CPU缓存一致性（某一数据被多个物理核的cache缓存，某一时刻其中一个物理核更新了该数据，更新后的值首先会被写入该核心自有的cache，此时需要通知其他缓存了该数据的核心他们缓存的值已经失效）

### std::thread（C++11）
线程API  
`hardware_concurrency()`：获取当前CPU逻辑核心数，以此确定线程池的工作线程数量  

`join()`：等待被创建线程的结束，并回收它的资源（避免资源泄露）（资源管理类的析构函数中调用）  
