[TOC]
# 设计模式
把“变化”限制在局部的地方（管理变化、提高可复用性）（隔离变化）  
手段：分解 + 抽象  

八大原则：
- 依赖倒置原则
  - 稳定的高层模块不应该依赖于变化的低层模块，二者都应该依赖于抽象（稳定）；抽象不应该依赖于实现细节（变化）
- 开放封闭原则
  - 对扩展开发、对更改封闭（类模块应该是可扩展而不可修改的）
- 单一职责原则
  - 一个类应该仅有一个引起变化的原因，这个变化的方向隐含对应着这个类的责任
- Liskov替换原则
  - 继承关系意味着子类能够替换基类（IS-A关系）（继承能够表达抽象）
- 接口隔离原则
  - 接口应该小而完备，客户程序不应该依赖于它们不用的方法
- 对象组合优于类继承
  - 继承某种程度上破坏了封装性（子类和父类耦合度高）；而对象组合只要求被组合的对象具有良好定义的接口，耦合度低
- 封装变化点
  - 使用封装来创建对象之间的“分界”；开发者可以在一侧修改，而不影响另一侧
- 面向接口编程
  - 业务问题声明为接口，而不是声明为具体类，客户程序无需获知具体类，只需调用接口
