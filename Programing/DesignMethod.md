...
定义一个操作的**算法骨架（稳定）**，将一些**具体步骤（变化）延迟到子类中**；使得子类可以在不改变算法的结构即可重定义某些特定步骤  

稳定的框架，其中步骤调用多态的虚函数  
框架类里实现主流程，并维护一个应用类的基类指针   

<br>

---
### Strategy 策略模式
代替 if-else 结构（扩展if-else结构需要重新编译），各类算法独立变化，与使用者分离  

通过一个对应条件分支策略的基类，**各分支对应不同的子类**  
框架类维护一个基类指针，在构造函数中通过参数传递让其指向具体应用对应的派生类   
...
### Observer 观察者模式
基于事件的UI框架  
一对多的“通知”依赖关系（当一个对象状态更新时，所有依赖于它的对象都得到通知并自动更新）  
定义一个抽象通知机制（“通知”的抽象接口）`Observer`，其子类`ConcreteObserver`为具体的通知控件  

对于引发通知的对象`Subject`，维护一个保存它所有观察者的数据结构  
通过多态指针调用具体的“通知方法”   
并定义一个 订阅/取消订阅的机制 参数为抽象类指针`Observer`  
订阅时`ConcreteObserver`维护一个指向`ConcreteSubject`的指针，以便获取其内部状态（在获取内部状态并进行处理的过程相对复杂时）  

*C++的多继承推荐只继承一个具体类，其他都是接口*  
