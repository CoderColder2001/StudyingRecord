...
提出了 对偶空间离散diffusion（SD3）框架 同时考虑SI2T和ST2I  
利用3D->X过程的中间特征来指导困难的X->3D的过程  
...
Spatial Dual Discrete Diffusion由三部分构成：1、3DSG生成；2、SI2T生成；3、ST2I生成  
在对称学习过程中，SI2T和ST2I首先通过一个共享的graph diffusion分别从输入图像或文本中诱导出3DSG表示，在3DSG中同时保留模态变异特征，用于三维空间场景的无偏和整体建模  
...
在ST2I过程中，语言prompt的抽象性质只允许对内容进行一般的描述，而不是对空间场景的详细描述  

SI2T和ST2I是对称过程，每个任务的输入和输出都是另一个任务的反向  

Scene Graph在描绘图像或文本中场景的内在语义结构方面取得了重大进展；SG中，关键对象和属性对应的node通过成对关系连接来描述语义上下文  
...
### 3D Scene Graph
3DSG用层级组织起来；包含更丰富的元素，如高级空间概念的节点  
...
---
## （2024）FastLGS: Speeding up Language Embedded Gaussians with Feature Grid Mapping
  
keyword：语义场；3D场景理解；3D高斯；open-vocabulary    

**任务：自然语言交互式地生成、提取场景目标**  

提出了用 **语义特征网格** 来保存基于片段任意模型（SAM）mask提取的多视图CLIP特征，并将网格映射到低维特征，通过3DGS进行语义场训练  
训练完毕后，对于开发词汇查询，从渲染像素级特征并通过 语义特征网格 恢复像素对齐的CLIP特征用于查询  

FastLGS语义更准确、更快，且可以以可调的方式查询具有类似查询的单个或多个对象

FastLGS首先提取所有的SAM掩码，通过CLIP编码器提取对象级的图像特征   
由于原始CLIP特征太大，计算代价高昂，构建一个特征网格来将CLIP特征映射到较低维的3D空间，但这带来了一个“多对一问题”；对此，提出了一个 **交叉视图特征匹配策略**  
将低维映射附加到高斯上，以建立一个特征域  

<br>

### Background：  
语义交互式的辐射场有潜力促进用户友好的、自动化的现实世界3D场景理解应用   

现有方法几乎只关注区分物体的准确性；它们的标签和低水平特征（例如DINO特征）不足以进行自然模态的相互作用，因为一些方法需要额外的位置信息  

使用具有高级特征的预训练的文本图像模型（如CLIP）的方法（如LERF）具有更好的可交互性与zero-shot能力；但LERF的结果不是以对象为中心的（无法“选取出”特定的区域或对象），只能输出相关性图（而这可能对于不同视角，不是3D一致的）   

*在构建语义场时，平衡 开放词汇能力 和 定位（分割）准确性 的困境*  

原始SAM无法处理不同视图各个masks之间的对应关系  

在查询上，如果像在LERF中那样通过计算每个像素的CLIP相关性来查询，其计算开销仍然是负担不起的   

<br>

### 问题：
1、直接在高斯上使用CLIP特征：空间开销大、渲染效率低，且有多视角不一致的问题（冗余的背景和复杂的遮挡关系）；如何压缩？如何统一多视角？   


<br> 

### 语义特征网格
由于来自多个视图的同一对象的语义特征应该共享相似的语义，因此可以压缩它以降低训练和查询成本  
但基于MLP的压缩和重构特征（如LangSplat）不能提供更高质量的查询，因为它们可能像原始特征一样不一致和不准确  

语义特征网格：存储场景中每个对象的多视图语言特征，并映射到一个低维特征  

<br>

### 交叉视图网格映射
匹配相邻图像集中的去噪masks，并分配特征，得到场景中的物体数量`K`   
匹配过程包括 **关键点对应** 和 **特征相似度比较**  

关键点对应：利用 SIFT 和 KNN算法 获得的关键点对mask correspondence进行优先排序，以实现精确和鲁棒匹配  
（超过𝜏个对应的关键点对的masks共享同样的低维特征）  

特征相似度比较：（源于有些mask比较平滑，关键点少）根据CLIP特征`L`和颜色分布`C`的混合特征计算相似度  
$sim_{i,j}=\alpha sim^{color}_{i,j}+(1-\alpha) sim^{CLIP}_{i,j}$  
其中颜色分布之间采用 巴氏距离，CLIP特征之间采用 余弦相似度  
...
### 查询
基于一个相机视角，先渲染得到2D语义特征图  

对于给定的prompt，采用类似LERF的方法计算相关性得分  
通过特征网格来恢复高维的图像特征，并使用它们计算与text embedding的相关性   
由于每个grid都有一个对象的多视图图像特征，可以在不体现这个特征的视图中查询到对应的物体（如在一本书的背面视角查询到名为xxx的书）  

计算grids和像素渲染特征的欧氏距离，小于一定阈值的像素点构成mask  

通过改变所使用的grid的数量，可以可调整地查询多个目标  

下游应用上，与SAGA结合并同时训练SAGA分割特征，查询得到的2Dmask可作为3D分割的prompt  

<br>

### 思考
不太明白 grip map的原理（为什么能work）  
查询好像还是在2D上做的，只得到2Dmask  
