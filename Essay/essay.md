...
添加一个额外的卷积层，以提高ViT模型产生的密集特征图的分辨率和一致性  
*（ViT将图像tokens 以 patches 形式处理，从而导致更低分辨率的特征图）*    
获得高分辨率像素特征的标准方法是采用线性插值  
卷积层使得模型于插值前在相邻patches间显式地交换信息  

在损失函数上，对比性损失并不直接优化对应关系；采用SmoothAP loss（直接优化了排名损失，从而提高了相应像素之间的特征检索的平均精度）  
