...
------
## （2024CVPR）Segment Any 3D Gaussians
  
keyword：分割；3D高斯；    

通过 2D点的prompt 对3D高斯进行分割  
...
Pipeline：  
首先获取每张图像的所有SAM masks，对于每个mask $M_I^i$，其三维物理尺度$s_{M_I^i}$使用预测相机的深度计算；再使用有尺度感知能力的对比训练  
对于给定视角，将2D point prompt（以及尺度）转换为相应的 3D scale-gated查询特性   
...
将高斯的关联特征通过不同的“门”（表示尺度的向量）投影到不同尺度的特征子空间中  
