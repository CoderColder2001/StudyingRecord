...
合并条件基于两个高斯分布间的Bhattacharya距离和平面描述符之间的余弦相似度  
Bhattacharya距离：  
...
先将高斯原语分组为局部聚类簇，以簇作为GMT的叶节点  
为进行局部分组，在优化过程中为每个GS引入两个附加参数：GS中心的表面法向量、附加的可学习平面描述符向量（用来识别不同的三维平面）  
...
再使用一个线性层（`W`）来将这个描述符图像中的每个像素映射到一个编码一个划分的one-hot向量$y\in{0,1}^m$（通过描述符`z`预测`y`）  
...
**使用区域邻接图（RAG）进行融合**  
*SAM可能将一个平面划分为不同的块，不能直接用SAM的结果作为 label $Y$;需要融合属于同一个平面的2D划分*  
RAG的节点表示SAM给出的划分，用边连接2D图像中对应区域相邻的节点  
...
为保持对于各个平面描述符z的整体可分离性，每N次迭代（2000轮后的每100轮）根据下式进行几轮（10轮）z的更新：  
<img src="./pic/PGS_4.png">   

<br>

### 思考
平面实例的聚类依靠RAG分割？（依然要取阈值：平面距离10cm、法向量夹角30度）   
