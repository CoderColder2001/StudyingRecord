...
**任务：LLM增强的场景图学习，将物品归位整理放置**（需要“物体间的功能关系”知识）   
affordance graph learning：在LLM帮助下，基于 上下文分析 和 常识推理 对场景affodances进行深度挖掘，实现高级的场景分析  
结合LLM评分器实现下游应用  
*（除 “tidy the room”以外，通过不同的prompt（如修改为“find a suitable place for a human activity”，并用具体活动替代可携带体的affordance描述），可以实现对“人-场景”互动关系的理解）*   

给定SG与各个物体节点对应的关键帧，利用LLM对场景中的所有对象进行上下文诱导的affordace分析  
使用LLM，将输入SG转换为具有 信息增强节点 和 新发现的边（关系）的affordance增强图（AEG）  
...
### 问题：
1、如何使LLM与使用者偏好对齐？  
2、如何得到物体affordance？如何使用LLM增强SG以及相关应用任务？（局部上下文分析）  
3、初始场景图只处理里邻近物体，如何建立较远物体之间的关系？（全局上下文分析；再用全局上下文增强SG的节点属性与边）  
4、如何保持LLM的客观性和有效性的评分？（在prompt中写评分规则）  
...
**全局上下文分析**（在远距离物体间建立边）  
**建立S的层级结构，用以分析空间上距离较远的物体间的关系**  
这一过程新加了边、更新了各物体节点中存储的信息，得到affordance-enhanced graph（AEG）  

“object-area-room”层级：对象首先被分组到一个称为“区域”的中间粒度级别中，然后进一步合并到一个房间中  
- 首先，使用LLM分别聚合每个area的上下文信息（得到description和fine-grained name）  
  - 每个area都将与上述 上下文信息 及 该区域内的对象列表 相关联  
- 随后，整合room内所有area信息，并将其作为房间的全局上下文，以为房间内所有容器和固定物体提取远距离上下文  
- 对于每个容器，输入基本的空间信息template prompt、其局部affordance分析以及全局上下文到LLM，得到远距离上下文（SG中新的语义边）
  - 上下文输出将以对象列表的格式出现：指示房间内与给定容器的功能相关的对象，以及这些对象和给定容器之间的特定功能关系

局部上下文分析与全局上下文分析结束后，再使用LLM更新上下文的功能信息（其输出格式与局部上下文分析相同）  
 
<br>

### 错误放置检测与重排
**检测：**  
基于LLM的评分器，根据物体对间的affordance和description  

由于LLM的token数限制，逐一进行评分  
取50分（0-100）作为阈值  

**重排：**  
直接将AEG中所有可能的容器的affordance输入给LLM，不仅会带来过多不相关信息，损害LLM推理能力，还会使其失去注意力，增加出现幻觉的可能性  
受检索增强生成（RAG）启发（*从给定的数据库中检索与任务相关的文本块，以增强LLM的任务解决能力，防止LLM幻觉*）  
构建容器affordance数据库（AEG中所有容器节点的description），从中（使用LLM评分器）检索与任务相关的top-k容器作为候选，发送到LLM以选择最佳目标   
（过滤与任务无关的容器，以降低LLM最终决策的难度）  
将候选容器的上下文诱导的affordance以及查询（任务中要重排的可携带体的description）输入LLM放置决策生成器，输出最佳放置目标  

<br>

### 场景图构建
根据给定的RGB-D帧，先重建场景点云，然后使用OVIR-3D进行实例分割，得到各个 2D mask 并提取CLIP特征   
再将mask和CLIP特征投到相应的点云区域上，得到3D候选区域  
通过比较 <b>IOU</b>（bbox？） 和 <b>特征相似度</b> 来合并相似的3D候选区域  

SG属性：  
每个3D实例为SG中的一个node，该实例的ID作为名称  
在ImageNet21K数据集中找到与实例CLIP特征最相似的对象标签  

计算每个3D实例在所有RGB-D帧中的可见像素数，并创建 “对象-图像像素” 矩阵$P \in R^{N*M}$，后续用于房屋标签和关键帧选取    

根据使用这些3D实例bbox，计算两种类型的空间关系：“near”（通过空间距离阈值）和“on-support”（需要判断对象间的垂直空间关系）  
above：两个物体在XY平面上投影bbox的2DIoU大于80%，而3DIoU为0  
on/support：相对高度差小于0.2米  

由于基于RGB-D图像构建SG，并且所看到的对象通常不在其他对象内部，因此不考虑“in”关系  

关系在SG中为有向边，作为节点对的环境属性的表示  

使用谱聚类方法得到rooms中的areas  

场景图 affordance-enhanced：  
<img src="./pic/LLM-enhanced-SG_1.png">  

<br>

### 思考
怎么从LLM的文本输出中提取出“最终目标”的？  
...
------
## （2024 CVPR）LEGaussians: Language Embedded 3D Gaussians for Open-Vocabulary Scene Understanding
...
### 问题：
1、如何**提取与存放语义特征**？直接向3D高斯嵌入语言特性会导致高昂的内存使用和性能下降（不在3D高斯上嵌入原始语义特征；而提出了一种新的特征量化方法，利用 **局部语义特性的冗余特性**，构造更精简的语言特征，并紧密地存储在3D高斯上）  
2、如何解决多视图不一致导致的语义歧义问题？（实现一种基于学习不确定性值的指导下降低语义特征空间频率的机制）  

<br>

### 语义特征的提取与处理
为提取像素级的语义特征，采用了一种略微不同的分层随机裁剪技术来提取CLIP特征，类似3D-OVS   
从CLIP中提取的特征只提供了不同语义区域的粗略边界；再提取了DINO特征 作为补充，以增强所提取的语言特征的细节  
对于像素`(x, y)`，将从多视图图像中提取的密集CLIP和DINO特征<b>连接</b>，作为最终的混合语言特征映射  
...
在继承原始CLIP的开放语义特征的同时，不需要额外的网络模块进行特征维数压缩或量化  

提出一个两阶段过程将语义特征（实例特征）从粗到细进行离散化，从而产生离散的三维实例集群  
基于IoU和特征距离的一种实例级的3D-2D特征关联方法，将3D点链接到2D masks，并进一步与个三维实例的多个视图的CLIP特征进行关联  

查询时，筛选与文本具有高相关性的3D高斯点进行渲染  

训练完实例特征后离散化到各个叶子节点存储&对应语义？  

<br>

### Background：  
神经渲染被证明是2D图像和3D场景之间的有效连接方法    
...
同时，而写2D像素级的方法：无法识别被遮挡的物体或部件，且语义依赖渲染图导致了无法在3D上实现交互，与机器人技术和实体智能应用程序不兼容（这些需要点级理解）  
...
### 实例特征训练
用一个低维特征$f\in R^6$增强每个三维高斯点，以表示其实例属性  

使用没有交叉视图相关性的二进制SAM mask（而非高维SAM特征）监督渲染的实例特征：通过masks内平滑损失和masks间对比损失  

基于3DGS的alpha-blending渲染实例特征图  

<br>

### 两阶段编码表离散化
由于特征映射是使用进行了权重累积的alpha混合渲染的，来自同一对象的高斯不可避免地表现出不同的特征，而来自不同对象的高斯可能具有相似的特征  

通过离散化确保来自同一实例的高斯特征具有相同（而不仅仅是相似）的特征  

从所有高斯中随机选择`k=64`个特征，以初始化编码表  
在训练中迭代优化编码表   

第二阶段中，仅根据实例特征在每个粗集群内进一步离散化  

### 获取语义特征
渲染到某一视图后，联合IoU和特征相似性的原理，将3D点与2D mask关联起来  

在查询时，对于所有实例，分别将单一实例渲染到当前视角，计算实例与当前视角各个SAM mask的IoU以及构造的特征距离，从而得到当前视图下最匹配的SAM mask，以其CLIP特征作为语义特征  
...
---
## （2024）Planar Gaussian Splatting
  
keyword：3DGS；实例分割；平面重建   

**任务：**   
在<b>树状高斯结构</b>中组织高斯场景，相似的GSGS逐步概率合并，以识别不同的三维平面实例，并形成整体的三维场景  
不需要平面标签与深度监督  

用一组高斯原数表示平面，附带可学习的平面描述符，并与其他高斯参数共同优化  

基于SAM辅助高斯的分组  
通过建模和解一个线性回归问题，然后使用区域邻接图（RAG）合并段，来解决诸如缺乏多视图masks之间关联和可变数量的masks等挑战   
利用概率方法来分组特征  

<br>

### Background：  
很多方法需要预定义平面数量的限制，或者需要GT的深度信息，或依赖平面标注进行训练   

NMF利用聚类方法作为后处理方法，对平面或对象实例的特征进行分组，效果依赖于不同场景不同的聚类超参数  
...
### 高斯混合树 Gaussian Mixture Tree
**采用高斯混合模型（GMM）来建模3D高斯场景以识别不同的平面实例**  
连接节点并推导出中间父节点  
在GMT中，根的子节点代表单个的3D平面实例  

树中的每个父节点（除了根节点）在三维空间中指定一个高斯分布，在空间中包含其子高斯节点的所有中心点  
合并高斯点得到父高斯点：  
$\sum_p=\sum_j·(\sum_i+\sum_j)^{-1}·\sum_i$  
$\mu_p=\sum_j·(\sum_i+\sum_j)^{-1}·\mu_i + \sum_i·(\sum_i+\sum_j)^{-1}·\mu_j$  
合并条件基于 <b>两个高斯分布间的Bhattacharya距离</b> 和 <b>平面描述符之间的余弦相似度</b>  
Bhattacharya距离：  
<img src="./pic/PGS_1.png">  

GS合并伪代码：  
<img src="./pic/PGS_2.png">  

不直接在叶节点上使用高斯原语（即通过高斯喷溅优化获得的参数），否则计算代价高  
而是 **先将高斯原语分组为局部聚类簇，以簇作为GMT的叶节点**  
为进行局部分组，在优化过程中为每个GS引入两个附加参数：GS中心的表面法向量$n\in R^3$、附加的可学习平面描述符向量$z\in R^k$（用来识别不同的三维平面）  

<br>

### 平面描述符学习（将2D学习到3D）
使用现成的网络 Omnidata model 来预测二维训练图像的法向图  
高斯渲染得到的法向量与模型预测的法向量计算损失：  
$L_n=\sum_{(u,v)} (1-<\hat{n_{uv}}·n_{uv}>), \hat{n_{uv}}=\sum_{i}^N n_i\alpha_i \prod_j^{i-1}(1-\alpha_j)$  
...
限制描述符`z`为单位向量  
提出了一种线性回归方法（具有闭式解）来预测每个训练图像基于`z`的二维划分的indices目录  
基于高斯渲染管线，从3D描述符渲染出2D描述符的渲染图  
再使用一个线性层（`W`）来将这个描述符图像中的每个像素映射到一个编码某个SAM划分的one-hot向量$y\in\{0,1\}^m$（通过描述符`z`预测`y`）  

求`W`的闭式解：  
<img src="./pic/PGS_3.png">  
（训练平面描述符`z`）  
其中$y_i$是对应实际划分标签one-hot向量$Y$、$Z$分别为代表标签和描述符的矩阵   
为每个给定的相机视图重新计算`W`的线性解   
不同视图有不同的`m`（$y_i$的维数）  

如何得到某个图像对应的`Y`？  
**使用区域邻接图（RAG）进行融合（SAM mask）**  
*由于SAM可能将一个平面划分为不同的块，不能直接用SAM的结果作为 label $Y$;需要融合（一个图像内）属于同一个平面的2D划分*  
RAG的节点为 SAM mask，初始化时，用边连接2D图像中对应mask区域相邻的节点  
通过切割两个不同平面的连接边（使用 <b>法向量</b> 和 <b>平面距离</b>）来划分RAG，并保持同一平面的节点间的连通性  
- 获取法向量：通过GS渲染管线
- 获取平面距离：假设每个像素属于场景中的一个平面表面，对于一个带法向量的3D点，其平面的点法式方程为$\mathbf{n·p}+d_p=0$  
  - 对于另一像素点`(u,v)`：$d_p=d_{(u,v)}·(\frac{n_1}{f_x}(u_0-u)+\frac{n_2}{f_y}(v_0-v)-n_3)$，其中$(f_x,f_y)$是pinhole相机的焦距，深度$d_{(u,v)}$由GS渲染得到  

每个SAM划分内的 平均法线距离 和 平局平面距离 被分配给RAG中相应的节点；设定阈值后对RAG进行切边  

<br>

### 3DGS训练中的平面约束
原始3DGS对高斯的空间排列没有任何显式的约束  
alpha-blending的监督不是单独针对每个实例  
被遮挡的GS受到较弱的监督  
影响了3D法向量和表面描述符的学习  

将GS对齐到平面（投影高斯分布的中心在其局部切平面上）：  
计算GS的KNN 以及 场景中KNN中心的协方差矩阵  
给定局部协方差3×3矩阵，利用协方差矩阵的两个最大特征值对应的两个特征向量进行奇异值分解  
基于KNN，对学习得到的 法向量 和 描述符特征 应用拉普拉斯平滑处理  
（每500次迭代应用一次局部平面对齐）  
...
### 思考
RAG分割？（依然要取阈值：平面距离10cm、法向量夹角30度）   
平面实例的聚类受顺序影响？（阈值？）伪代码流程有点没看懂  
聚类合并的时候怎么更新z？  

<br>

---
## （2024）CogNav: Cognitive Process Modeling for Object Goal Navigation with LLMs
  
keyword：Scene Graph；目标导航；空间关系；场景分析；LLM-enhanced  

**任务：开放词汇场景导航**   
输入：目标类别c、RGB-D帧序列  

异构认知地图：场景图S + 地标图L + 占用率俯视图M（当前位置、已勘测位置、未勘测位置等）

用VLM推断观察结果，优化SG中节点和边的描述  
...
### 问题：
1、直接从聚类获得的分割结果往往有噪声且容易出错？（使用VLM推理优化） 

###  Cognitive Map 构建
**场景图：**  
利用深度图和相机位姿投回3D，实现三维实例分割  
每一步实例与现有实例群$O_{t−1}$合并，通过DBSCAN聚类更新实例节点集合$O_t$  

基于VLM优化：（采用SoM技术）用$O_t$的投影标记出二维实例，然后对空间信息和语义信息进行推理，如房间类型  
每`10`步使用VLM推理优化，只作用于新融合进来的节点和相应的边  

### Cognitive Map Prompting
对于每个landmark，集成场景信息（周围的对象及其关系、房间类型和边界属性）  

### LLM使用
**在新环境&开放词汇目标下，定义严格的约束或阈值来触发这些转换具有挑战性,因此使用LLM进行状态转移**  
